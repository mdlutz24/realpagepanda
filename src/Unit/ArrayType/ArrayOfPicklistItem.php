...
class ArrayOfPicklistItem extends AbstractStructArrayBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \mdlutz24\realpagepanda\Unit\StructType\PicklistItem[]
     */
    public $PicklistItem;
    /**
     * Constructor method for ArrayOfPicklistItem
     * @uses ArrayOfPicklistItem::setPicklistItem()
     * @param \mdlutz24\realpagepanda\Unit\StructType\PicklistItem[] $picklistItem
     */
     ...
     * @return \mdlutz24\realpagepanda\Unit\StructType\PicklistItem[]|null
     */
    ...
    public static function validatePicklistItemForArrayConstraintsFromSetPicklistItem(array $values = array())
    {
        ...
        foreach ($values as $arrayOfPicklistItemPicklistItemItem) {
            ...
            if (!$arrayOfPicklistItemPicklistItemItem instanceof \mdlutz24\realpagepanda\Unit\StructType\PicklistItem) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PicklistItem property can only contain items of type \mdlutz24\realpagepanda\Unit\StructType\PicklistItem, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \mdlutz24\realpagepanda\Unit\StructType\PicklistItem[] $picklistItem
     * @return \mdlutz24\realpagepanda\Unit\ArrayType\ArrayOfPicklistItem
     */
     ...
     * @throws \InvalidArgumentException
     * @param \mdlutz24\realpagepanda\Unit\StructType\PicklistItem $item
     * @return \mdlutz24\realpagepanda\Unit\ArrayType\ArrayOfPicklistItem
     */
    public function addToPicklistItem(\mdlutz24\realpagepanda\Unit\StructType\PicklistItem $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \mdlutz24\realpagepanda\Unit\StructType\PicklistItem) {
            throw new \InvalidArgumentException(sprintf('The PicklistItem property can only contain items of type \mdlutz24\realpagepanda\Unit\StructType\PicklistItem, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
